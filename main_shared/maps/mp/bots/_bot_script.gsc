...
set_class(rankxp)
{
	...
	for(i=0; i < 5; i++)
	{
		...
		att1 = get_random_attachment(primary);
		...
		att2 = get_random_attachment(secondary);
...
bot_weapon_think()
{
	...
	for(;;)
	{
		...
		while(weaponslist.size)
		{
			...
			if(curWeap == weapon || weapon == "c4_mp" || weapon == "none")//c4 no work
				continue;
...
bot_revenge_think()
{
	...
	went = isDefined(self.killerLocation);
